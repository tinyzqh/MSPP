...
class TransitionModel(nn.Module):
	...
	def forward(self, prev_state: torch.Tensor, actions: torch.Tensor, prev_belief: torch.Tensor, observations: Optional[torch.Tensor] = None, nonterminals: Optional[torch.Tensor] = None) -> List[torch.Tensor]:
		...
		if args.MultiGPU and torch.cuda.device_count() > 1:
			...
			observations = None if observations is None else torch.transpose(observations, 0, 1)
			nonterminals = None if nonterminals is None else torch.transpose(nonterminals, 0, 1)
		# Create lists for hidden states (cannot use single tensor as buffer because autograd won't work with inplace writes)
		T = actions.size(0) + 1
		beliefs, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [torch.empty(0)] * T, [
...
def Encoder(symbolic, observation_size, embedding_size, activation_function='relu'):
	if args.MultiGPU:
		if symbolic:
			SymbolicEncoder_model = SymbolicEncoder(observation_size, embedding_size, activation_function)
			return nn.DataParallel(SymbolicEncoder_model.cuda())
		else:
			return nn.DataParallel(VisualEncoder(embedding_size, activation_function).cuda())
