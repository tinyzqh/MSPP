...
class Plan(object):
  ...
  def __init__(self):
    ...
    print("We Have {} GPUS".format(torch.cuda.device_count())) if torch.cuda.device_count() > 1 and args.MultiGPU else print("We use CPU")
    self.transition_model = nn.DataParallel(self.transition_model.to(device=args.device)) if torch.cuda.device_count() > 1 and args.MultiGPU else self.transition_model
    self.observation_model = nn.DataParallel(self.observation_model.to(device=args.device)) if torch.cuda.device_count() > 1 and args.MultiGPU else self.observation_model
    self.reward_model = nn.DataParallel(self.reward_model.to(device=args.device)) if torch.cuda.device_count() > 1 and args.MultiGPU else self.reward_model
    ...
    if args.overshooting_kl_beta != 0:
      ...
      if args.overshooting_reward_scale != 0:
        reward_loss += (1 / args.overshooting_distance) * args.overshooting_reward_scale * F.mse_loss(
          bottle(self.reward_model, (beliefs, prior_states)) * seq_mask[:, :, 0], torch.cat(overshooting_vars[2], dim=1),
          reduction='none').mean(dim=(0, 1)) * (args.chunk_size - 1)  # Update reward loss (compensating for extra average over each overshooting/open loop sequence)
  ...
  def train(self):
    ...
    for s in tqdm(range(args.collect_interval)):
      ...
      beliefs, prior_states, prior_means, prior_std_devs, posterior_states, posterior_means, posterior_std_devs = self.upper_transition_model(prev_state=init_state, actions=actions[:-1], prev_belief=init_belief, obs=obs, nonterminals=nonterminals[:-1])
