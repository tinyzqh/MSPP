...
def Save_Txt(xs, ys_population, title, path='', xaxis='episode'):
    filename = os.path.join(path, title + "_" + xaxis+ '.txt')
    if isinstance(ys_population, list) or isinstance(ys_population, tuple):
        ys = np.asarray(ys_population, dtype=np.float32)
        ys_min, ys_max, ys_mean, ys_std, ys_median = ys.min(), ys.max(), ys.mean(), ys.std(), np.median(ys)
        ys_upper, ys_lower = ys_mean + ys_std, ys_mean - ys_std
        datas = [xs, ys_min, ys_max, ys_mean, ys_std, ys_median, ys_upper, ys_lower]
        if not os.path.exists(filename):
            with open(file=filename, mode="a+") as file:
                file.write("xs ys_min ys_max ys_mean ys_std ys_median ys_upper ys_lower")
                file.write("\r\n")
    else:
        datas = [xs, ys_population]
        if not os.path.exists(filename):
            with open(file=filename, mode="a+") as file:
                file.write("xs ys_population")
                file.write("\r\n")

    with open(file=filename, mode="a+") as file:
        for index, data in enumerate(datas):
            file.write(str(data)) if index==0 else file.write(" "+str(data))
        file.write("\r\n")
...
def imagine_ahead(prev_state, prev_belief, policy, transition_model, planning_horizon=12, action_scale=1):
    '''
...
def get_modules(modules):
  ...
  for module in modules:
    if hasattr(module, "model_modules"):
      module_res = module.model_modules
    else:
      module_res = module.module.model_modules
