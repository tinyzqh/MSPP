...
if __name__ == "__main__":
  ...
  if torch.cuda.device_count() > 1 and args.MultiGPU:
    ...
    transition_model = nn.DataParallel(transition_model.cuda(), device_ids=[0, 1, 2])
    observation_model = nn.DataParallel(observation_model.cuda(), device_ids=[0, 1, 2])
    reward_model = nn.DataParallel(reward_model.cuda(), device_ids=[0, 1, 2])
    # encoder = nn.DataParallel(encoder.cuda(), device_ids=[0, 1, 2])
    actor_model = nn.DataParallel(actor_model.cuda(), device_ids=[0, 1, 2])
    value_model = nn.DataParallel(value_model.cuda(), device_ids=[0, 1, 2])
